...
void CSE7766Component::parse_data_() {
  ...
  float voltage = 0.0f;
  if (have_voltage) {
    voltage = (voltage_coeff * 1.883f) / float(voltage_cycle);
  ...
  if (this->energy_sensor_ != nullptr) {
    ...
    this->cf_pulses_last_ = cf_pulses;
    energy = this->cf_pulses_total_ * float(power_coeff * 1.883f) / 1000000.0f / 3600.0f;
  ...
  if (power_cycle_exceeds_range) {
    ...
    power = (power_coeff * 1.883f) / float(power_cycle);
    if (this->power_sensor_ != nullptr) {
